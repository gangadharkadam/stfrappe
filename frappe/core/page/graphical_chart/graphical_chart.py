...
def get_sales_pie(from_date=None,to_date=None,currency=None,country=None):
	...
	if from_date and not to_date:
		raise RuntimeError(' Please renter to  date')                           
...
def get_data_newsalecol(from_date=None,to_date=None,country=None):
	...
	if to_date:
		to_date=to_date[6:] + "-" + to_date[3:5] + "-" + to_date[:2]
	if country and (not from_date and not to_date):
		frappe.errprint("only country ")	
		str1="select substring(date_format(l.creation,'%Y-%M'),1,8)as month,count(distinct l.name) as lead_count,count(distinct c.name) as cust_count from tabLead l left join tabCustomer c on  l.name=c.lead_name where l.creation between coalesce(null,'1111-09-01 14:49:40') and coalesce(null,'9999-09-01 14:49:40') and coalesce(l.country,0)=coalesce('"+country+"',coalesce(l.country,0)) group by date_format(l.creation,'%Y-%M')"
	elif (from_date and to_date) and not country:
		frappe.errprint("only date ")	
	...
	elif (not from_date and not to_date and not country):
		frappe.errprint("nothing ")	
	...
	frappe.errprint(str1)	
	new_sales_details=frappe.db.sql(str1)
	return{
...
def get_data_newsalepie(from_date=None,to_date=None,country=None):
	...
	if country and (not from_date and not to_date):
		frappe.errprint("only country ")	
		str1="select substring(date_format(creation,'%Y-%M'),1,8)as month,count(*) as lead_count from tabLead where creation between coalesce(null,'1111-09-01 14:49:40') and coalesce(null,'9999-09-01 14:49:40') and coalesce(country,0)=coalesce('"+country+"',coalesce(country,0)) group by date_format(creation,'%Y-%M')"	
	elif (from_date and to_date) and not country:
		frappe.errprint("only dates")	
	...
	frappe.errprint(str1)	
	sales_details=frappe.db.sql(str1)
	return{
...
def get_subscription(from_date=None,to_date=None,country=None):
	...
	if country and (not from_date and not to_date):
		frappe.errprint("only country ")	
		str1="select substring(date_format(expiry_date,'%Y-%M'),1,8)as month,count(*) as expiry_count from `tabSite Master` where expiry_date between coalesce(null,'1111-09-01 14:49:40') and coalesce(null,'9999-09-01 14:49:40') and coalesce(country,0)=coalesce('"+country+"',coalesce(country,0)) group by date_format(expiry_date,'%Y-%M') "	
	elif (from_date and to_date) and not country:
		frappe.errprint("only dates")	
		str1="select substring(date_format(expiry_date,'%Y-%M'),1,8)as month,count(*) as expiry_count from `tabSite Master` where expiry_date between coalesce('"+from_date+"','1111-09-01 14:49:40') and coalesce('"+to_date+"','9999-09-01 14:49:40') and coalesce(country,0)=coalesce(null,coalesce(country,0)) group by date_format(expiry_date,'%Y-%M')"	
	elif (not from_date and not to_date and not country):
		frappe.errprint("nothing")	
		str1="select substring(date_format(expiry_date,'%Y-%M'),1,8)as month,count(*) as expiry_count from `tabSite Master` where expiry_date between coalesce(null,'1111-09-01 14:49:40') and coalesce(null,'9999-09-01 14:49:40') and coalesce(country,0)=coalesce(null,coalesce(country,0)) group by date_format(expiry_date,'%Y-%M') "
	else:
		frappe.errprint("all else")	
		str1="select substring(date_format(expiry_date,'%Y-%M'),1,8)as month,count(*) as expiry_count from `tabSite Master` where expiry_date between coalesce('"+from_date+"','1111-09-01 14:49:40') and coalesce('"+to_date+"','9999-09-01 14:49:40') and coalesce(country,0)=coalesce('"+country+"',coalesce(country,0)) group by date_format(expiry_date,'%Y-%M') "	
	#frappe.errprint("new sale pie")		
	frappe.errprint(str1)	
	sales_details=frappe.db.sql(str1)
	return{
